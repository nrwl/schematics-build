...
function names(name) {
    return { name: name, className: toClassName(name), propertyName: toPropertyName(name), fileName: toFileName(name) };
}
exports.names = names;
...
function toPropertyName(s) {
    return s.replace(/(-|_|\.|\s)+(.)?/g, function (_, __, chr) { return chr ? chr.toUpperCase() : ''; })
